...
const _0x1dff8c = require("fs");
const _0x31c89b = require("path");
const main = require("./main-DSRivCYi.js");
...
class FileFromPath {
  constructor(input) {
    ...
    this.name = _0x31c89b.basename(__classPrivateFieldGet(this, _FileFromPath_path, "f"));
  ...
  async *stream() {
    const { mtimeMs } = await _0x1dff8c.promises.stat(__classPrivateFieldGet(this, _FileFromPath_path, "f"));
    if (mtimeMs > this.lastModified) {
      throw new DOMException(MESSAGE, "NotReadableError");
    }
    if (this.size) {
      yield* _0x1dff8c.createReadStream(__classPrivateFieldGet(this, _FileFromPath_path, "f"), {
...
async function fileFromPath(path, filenameOrOptions, options) {
  const stats = await _0x1dff8c.promises.stat(path);
